...
public class Game {
	...
	public static void main(String[] args) {
		...
		while ((n < 2) || (n > 4)) {
			System.out.println("Voulez-vous jouer à 2,3 ou 4 joueurs ?"); // On demande à combien de joueurs ils
																			// souhaitent jouer
		...
		case 2:
			...
			for (int i = 0; i < 24; i++) {
				...
				System.out.println(dominos.get(i).getCellule1().numero + " " + dominos.get(i).getCellule1().type + " "
						+ dominos.get(i).getCellule2().type);
			}
			break;
		// S'il y a 3 joueurs, on fait pareil que pour 2 mais avec 3 noms de joueurs à
		// saisir et 36 dominos sélectionnés aléatoirement
		case 3:
			int i = 1;
			while (i < 4) {
			...
			for (int t = 0; t < 36; t++) {
				...
				System.out.println(dominos.get(t).getCellule1().numero + " " + dominos.get(t).getCellule1().type + " "
						+ dominos.get(t).getCellule2().type);
			}

			break;
		// S'il y a 4 joueurs on fait pareil qu'avec 2 joueurs mais avec 4 noms de
		// joueurs à saisir et les 48 dominos mélangés
		case 4:
			int j = 1;
			while (j < 5) {
			...
			for (int f = 0; f < 48; f++) {
				...
				System.out.println(dominos.get(f).getCellule1().numero + " " + dominos.get(f).getCellule1().type + " "
						+ dominos.get(f).getCellule2().type);
			}
	...
	// On crée une liste dans laquelle on va ajouter les joueurs avec leur numéro,
	// leur nom et leur couleur
